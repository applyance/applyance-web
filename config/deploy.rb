...
namespace :travis do
	...
	task :create_release do
		run_locally do
			execute :mkdir, '-p', :'tmp'
			execute "tar -cz --exclude test --exclude vendor --exclude .git --exclude .sass-cache --exclude node_modules --exclude public/scripts/ext --exclude public/styles/css --exclude tmp/#{fetch(:release_timestamp)}.tar.gz -f tmp/#{fetch(:release_timestamp)}.tar.gz ."
		end
